...
SELECT customers.full_name, orders.order_date FROM customers
...
SELECT products.product_name, order_items.quantity, order_items.price_per_unit FROM order_items
JOIN products ON order_items.product_id = products.product_id WHERE order_items.order_id = 1;
...
) laptop_orders ON c.customer_id = laptop_orders.customer_id WHERE laptop_orders.customer_id IS NULL;
...
SELECT c.full_name AS customer_name, p.category AS product_category FROM customers c
CROSS JOIN (
    SELECT  category 
    FROM products 
) p
...
SELECT new_cust.full_name AS new_customer, rec_cust.full_name AS recommended_by FROM customers new_cust
LEFT JOIN customers rec_cust ON new_cust.recommended_by = rec_cust.customer_id
ORDER BY rec_cust.full_name NULLS FIRST, new_cust.full_name;